window.checkAndLaunchGoalConfetti = function () {
    const colors = ["#007bff", "#6c757d", "#28a745", "#17a2b8", "#ffc107", "#dc3545", "#f8f9fa"];
...
const goalIndex = start + goalAmount +1; // +1 to account for the current perek
    ...
    const currentIndex = parseInt(localStorage.getItem('globalPerekIndex') || '1') - 0;
    ...
    function launchConfetti() {
        ...
        for (let i = 0; i < max_particles; i++) {
            ...
            particle.style.width = particle.style.height = (Math.random() * 0.5 + 0.75) + 'em';
            particle.style.animation = `fall ${(Math.random() * 1 + 1.5)}s linear ${Math.random() * 2}s forwards`;
